...
#'
#' @examples
mpsplinet = function(obj, var.name, lam = 0.1, d = t(c(0,5,15,30,60,100,200)),
  ...
  objd_m <- objd[,c(grep(names(objd), pattern = glob2rx(paste0("^", idcol))),
  ...
  sel <- !(is.na(objd_m[, which(names(objd_m) == paste(var.name, "_A", sep = ""))])
  ...
  lw.hor <- as.vector(which(rowSums(!is.na(objd_m[ ,lowerb.lst])) <
                              rowSums(!is.na(objd_m[ ,upperb.lst])) &
                              rowSums(!is.na(objd_m[ ,upperb.lst])) > 1))  # profiles with un-even number of lower/upper depths
  ...
  for (st in as.vector(which(sel))) {
    subs <- matrix(unlist(c(1:np, as.vector(objd_m[st, upperb.lst]),
                            as.vector(objd_m[st, lowerb.lst]),
                            as.vector(objd_m[st, svar.lst]))), ncol = 4)
    d.ho <- rowMeans(data.frame(x = subs[,2], y = c(NA, subs[1:(nrow(subs) - 1), 3])),
                     na.rm = TRUE)
    ## mask out missing values
    if (ncol(as.matrix(subs[!is.na(subs[, 2]) & !is.na(subs[,3])
                            & !is.na(subs[,4]),])) == 1)
  ...
  retval <- list(idcol = objd_m[,1], var.fitted = dave, var.std = yave,
                 var.1cm = t(m_fyfit))
